    ...
    if (gui == TRUE) {
      ...
      calcular <- function(...){
        ...
        if (tclvalue(vari3) == gettext("Model", domain = "R-MCPtests")) {
          ...
          if (tclvalue(vari2) != "latex") {
            MCPtests::MCPwrite(x = envMCP$results, MCP = tclvalue(vari), extension = tclvalue(vari2), dataMR = 'all')
        ...
        if (tclvalue(vari3) == gettext("Response variable", domain = "R-MCPtests")) {
          objrv <- envMCP$dat[,tclvalue(vari7)]
          objtreat <- as.factor(envMCP$dat[,tclvalue(vari8)])
          capture.output(envMCP$results <- MCPtests::MCPtest(y = objrv,
                                                                  trt = objtreat,
        ...
        if (tclvalue(vari3) == gettext("Averages", domain = "R-MCPtests")) {
          ...
          # Functions
          capture.output(envMCP$results <- MCPtests::MCPtest(y = eval(parse(text = aver)),
                                                                  trt = trat,
