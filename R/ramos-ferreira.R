...
# anova(aov(y ~ trt))
# mserror <- 11.78867; dferror <- 28; alpha = 0.05
...
ramos_ferreira <- function(y, trt, dferror, mserror, replication, alpha, N = 1000, parallel = FALSE) {
  # Parallel activate
  if (parallel) {
  ...
  # Bootstrap distribution of Studentized range statistcs
  resample_srange <- function(...) {
    trt <- trt; replication <- replication; n <- n; y <- y
    if (parallel) {
      amostra <- foreach::foreach(i = 1:N, .combine = "c") %dopar% {
        trt <- as.factor(trt)
        r <- replication
        yb <- sample(x = y, size = n * r, replace = TRUE)
        anav2 <- aov(yb ~ trt)
        anav <- summary(anav2)
        means <- sort(model.tables(anav2, type = "means")$tables$trt)
        mse <- anav[[1]]$`Mean Sq`[2]
        dfe <- anav[[1]]$`Df`[2]
        q <- (max(means) - min(means)) / (mse / r)^0.5
      }
    } else{
      amostra <- foreach::foreach(i = 1:N, .combine = "c") %do% {
        trt <- as.factor(trt)
        r <- replication
        yb <- sample(x = y, size = n * r, replace = TRUE)
        anav2 <- aov(yb ~ trt)
  ...
  gdist = function(D, n){
    ...
    colnames(res) <- c("Step", "LL", 
                       "UL", "Range", "Stud. Range", "P-value")
    ct = 1
    ...
    arma.res <- function(ct, min, II, JJ) { 
      ...
      # Studentized range
      Q <- R / (sqrt(mserror/replication)) 
      res[ct,5] <- round(Q, 5)
      
      # Studentized range bootstrap statistics
      srb <- resample_srange()
      
      # P-value of statistics
      res[ct,6] <- (length(srb[srb > Q])) / N
...
# start_time <- Sys.time()
# ramos_ferreira(y = y, trt = trt, dferror = dferror, mserror = mserror, replication = replication, 
#                alpha, N = 1000, parallel = TRUE)
# end_time <- Sys.time()
# end_time - start_time
