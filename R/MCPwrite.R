#' Export the results of the \code{MCPtest} function.
#'
#' The \code{x} object from a \code{MCPtest} is written to file arguments.
#' @param x object from the \code{MCPtest} functions.
...
#' @export
MCPwrite <- function(x, MCP = "all", extension = "csv",
                    dataMR = "all"){
  ...
  if (any(dataMR == "groups")) {
    if(any(MCP == "MGM")) {
      ...
      if (extension == "latex"){
        ...
        dat  <- data.frame(trt, x[[2]][[1]])
        rownames(dat) <- 1:ntrt
        cat(gettext("Table in latex of results of the MGM test\n\n", domain = "R-MCP"))
  ...
  if (extension == "latex"){
    cat(gettext("\nSee yours tables in Console\n", domain = "R-MCP"),
        gettext("Format:", domain = "R-MCP"), extension)
  }
  if (extension != "latex"){
    cat(gettext("See your files in Directory\n", domain = "R-MCP"),
        gettext("Format:", domain = "R-MCP"), extension)
  }
}
