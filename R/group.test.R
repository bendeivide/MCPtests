...
group.test  <- function(Teste){
  ...
  conclusao <- data.frame(Means = w[, 1],
                          Groups = M)
  colnames(conclusao) <- c(gettext("Means", domain = "R-MCP"),
                           gettext("Groups", domain = "R-MCP"))
  return(conclusao)
}

# Function for to show the results of the MGM, MGR and SK tests
group.test2 <- function(Teste) {
  ...
  conclusao <- data.frame(Means = ordertest[, 1],
                          Groups = M)
  colnames(conclusao) <- c(gettext("Means", domain = "R-MCP"),
                           gettext("Groups", domain = "R-MCP"))
  return(conclusao)
